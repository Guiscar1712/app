...
module.exports = {
  ...
  UpdatePersonalDataValidate (model) {
    ...
    contract.isTrue(Util.isValidCpf(model.documentCpf), '40001', 'CPF é inválido')
    contract.isRequired(model.name, '40003', 'Nome é Obrigatório')
    contract.hasMinLen(model.name, 7, '40003', 'Nome deve ser completo')
    contract.isRequired(model.documentRg, '40003', 'Rg é Obrigatório')
    contract.isDateValid(model.birthdate, '40003', 'Data de nascimento inválida')
    ...
    if (model.telephones && model.telephones.length > 0) {
      ...
      model.telephones.forEach(element => {
        contract.isRequired(element.number, '40003', 'Numero de telefone é Obrigatório')
        contract.containsInArray(element.type, type, '40003', 'Numero de telefone é Obrigatório')
      })
    }
    ...
    if (model.addresses && model.addresses.length > 0) {
      ...
      model.addresses.forEach(element => {
        contract.containsInArray(element.type, tipo, '40003', 'Tipo de endereço é inválido')
        ...
        contract.containsInArray(element.stateAcronym, uf, '40003', 'Uf é invalido')
